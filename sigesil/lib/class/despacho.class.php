...
class Despacho extends Model {
    ...
    function listadoAnalisisD($idCA = null, $idDes = null, $estatus = null) {
        $query = "SELECT ca.id AS id_ca, ca.codigo AS codigo_ca, ca.nombre AS nombre_ca, 
                    ...
                    des.id AS id_rec, des.numero, des.estatus, des.fecha_des as fecha_recepcion, 
                    des.cant_muestras, des.carril, v.placa, v.placa_remolques 
                    FROM si_despacho des
                    INNER JOIN si_centro_acopio ca ON des.id_centro_acopio=ca.id
                    INNER JOIN si_cultivo cul ON cul.id=des.id_cultivo
		    INNER JOIN si_vehiculos v ON v.id = des.id_vehiculo
                    WHERE '1'";
        $query .= (!empty($idCA)) ? " AND ca.id = '$idCA'" : "";
        $query .= (!empty($idDes)) ? " AND des.id = '$idDes'" : "";
        $query .= (!empty($estatus)) ? " AND des.estatus = '$estatus'" : "";
        $query .= " ORDER BY ca.codigo, cul.id, des.fecha_des, des.numero";
    ...
    function despachosReporteGeneral($fdesde=null, $fhasta=null, $idCA=null, $porPagina=null, $inicio=null){
